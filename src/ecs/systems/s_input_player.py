...
def system_input_player(
    ...
    for _, c_input in components:
        ...
        if event.type == pygame.KEYDOWN and c_input.key == event.key:
            ...
            do_action(c_input)
        elif event.type == pygame.MOUSEBUTTONDOWN and c_input.key == event.type and event.button == 1:
            c_input.phase = CommandPhase.START
            do_action(c_input)
        elif event.type == pygame.MOUSEBUTTONUP and c_input.key == event.type and event.button == 1:
            c_input.phase = CommandPhase.END
            do_action(c_input)
